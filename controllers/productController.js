...
const findById = async (req, res) => {
  try {
    const products = await Product.find({ id: req.params.product_id }).populate(
      "category"
    );
    res.json(products);
  } catch (error) {
    res
      .status(500)
      .send({ status: false, message: "Error while fetching information..." });
    console.log(error);
  }
};
...
const save = async (req, res) => {
  try {
    ...
    if (!objNewProduct) {
      throw new Error("El cuerpo de la solicitud está vacío o no está definido.");
    }
    ...
    console.error(error);
    res.status(400).send(error.message); // Enviar mensaje de error al cliente
  }
};
...
const patch = async (req, res) => {
  try {
    const patchedProduct = await Product.findOneAndUpdate(
      { id: parseInt(req.body.id) },
      req.body,
...
const deleteProduct = async (req, res) => {
  try {
    const docDeleted = await Product.findOneAndDelete({
